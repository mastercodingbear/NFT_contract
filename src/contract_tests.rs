...
use cosmwasm_std::{
    from_binary, to_binary, Coin, CosmosMsg, DepsMut, Empty, OwnedDeps, Response, Uint128, WasmMsg,
};
...
const MINTER: &str = "honey";
const CONTRACT_NAME: &str = "Honey Pot";
const SYMBOL: &str = "HB";
...
fn minting_derived() {
    ...
    let err = contract
        .execute(deps.as_mut(), mock_env(), not_allowed, mint_msg_deriv)
        .unwrap_err();
    assert_eq!(err, ContractError::Unauthorized {});
}
